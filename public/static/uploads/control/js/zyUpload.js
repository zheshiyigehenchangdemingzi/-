	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : "/admin/product/uploads",  	// 上传文件的路径
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<a href=""><img class="upload_image" src="/static/uploads/control/img/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" /></a>';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "/static/uploads/control/images/fileType/";
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						var cover = $("#cover").val();
						$("#cover").val(cover + response.data.cover);
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
