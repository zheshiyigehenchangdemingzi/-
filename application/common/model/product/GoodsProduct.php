...
Class GoodsProduct extends Base
{
    // 条数
    public $count = 0;
            ...
            */
            ...
            if(empty($data['id'])){
                ...
                */
                ...
                $saveAllData = [];
                foreach($data['value'] as $k => $i){
                    $saveAllData[] = [
                        'value' => $i,
                        'sort' => $data['sort_val'][$k] - 0 == 0 ? $k : $data['sort_val'][$k] - 0,
                        'pid' => $data['id']
                    ];
                }
                $this->insertAll($saveAllData);
//                $this->insertData($data['value'],$save,$data['id']);
    ...
    public function getChild($id)
    {
        ...
        $childData = $this->where('pid',$id)->order('sort desc')->select()->toArray();
