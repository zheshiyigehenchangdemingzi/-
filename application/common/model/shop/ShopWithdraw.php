...
// 店铺详情
class ShopWithdraw extends Base
{
    // 获取列表
    public function getList($get){
        $where = ['w.is_delete'=>0];
        if(!empty($get['shop_id']))
            $where['w.shop_id'] = $get['shop_id'];
        ...
        if(!empty($get['start'])){
            ...
            $where['w.add_time'] = ['gt',$start];
        }
        if(!empty($get['end'])){
            $end = strtotime($get['end']."+1 day");
            if(!empty($start))
                $where['w.add_time']  =  ['between',[$start,$end]];
            else
                $where['w.add_time'] = ['lt',$end];
        }
        $field = 'w.id,w.total,w.amount,w.order_sn,w.rate,w.status,w.add_time,u.name';
        $list = $this->alias('w')->join('shop_users u','u.id=w.shop_id','left')->where($where)->field($field)->order('w.add_time desc')
            ->paginate(20,false,array('query'=>$get));
        $this->page = $list->render();
        $this->count = $this->alias('w')->join('shop_users u','u.id=w.shop_id','left')->where($where)->count();
        $sum = $this->alias('w')->join('shop_users u','u.id=w.shop_id','left')->where($where)->sum('total');
