...
class Videos extends Common
{
    ...
    public function index()
    {
        ...
        // 获取到省的数据
        list($provinceList,$cityList,$areaList) = $this->getRegionPcdList($get);
    ...
    public function edit()
    {
        ...
        if (request()->isAjax() && request()->isPost()) {
            ...
            $data = [
                ...
                'status' => 1,
                'is_hot' => 0,
                'type' => Video::DEFAULT_TYPE,
                'ad_type' => Video::AD_TYPE_DEFAULT,
                'ad_type_id' => Video::AD_TYPE_DEFAULT,
                'video' => '',
                'province' => 0,
                'city' => 0,
                'area' => 0,
            ...
            if ($id) $data = $model->getOne($id);
            $data['add_time'] = date('Y-m-d', isset($data['add_time']) ? $data['add_time'] : time());

            $data['cover'] = ImageSize::getStaticHostUrl($data['cover'] ?? '');
            $data['cover_show'] = ImageSize::cosThumbnail($data['cover'] ?? '');

            list($cityList,$areaList) = Region::getProvinceCity($data['province'],$data['city']);
            ...
            return $this->fetch('', [
                ...
                'count' => count($users),
                'type_list' => Video::$TYPE_TEXTS,
                'ad_type_list' => Video::$AD_TYPE_TEXTS,
                'province_list' => $provinceList,
                'city_list' => $cityList,
                'area_list' => $areaList,
            ]);
